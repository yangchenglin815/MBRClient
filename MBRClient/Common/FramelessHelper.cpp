...
void WidgetData::setMax(bool bMax)
{
    if(m_bMax != bMax)
    {
        ...
        {
            m_rect = m_pWidget->geometry();
            m_pWidget->setGeometry(qApp->desktop()->availableGeometry());
        }
        else
        {
...
{
    if (event->button() == Qt::LeftButton && d->m_bDoubleClickable)
    {
...
void FramelessHelper::setDbClickOn(bool on)
{
	d->m_bDoubleClickable = on;
}
...
bool FramelessHelper::bDClickOn()
{
	return d->m_bDoubleClickable;
}
