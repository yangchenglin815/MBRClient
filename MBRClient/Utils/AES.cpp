...
*******************************************************************/  
int AES::Run1Des(int bType, int bMode, const char *In, unsigned int in_len, const char *Key, unsigned int key_len, char* Out, unsigned int out_len, const char cvecstr[8])  
{  
    int i,j,k;  
...
*******************************************************************/  
int AES::Run3Des(int bType, int bMode, const char *In, unsigned int in_len, const char *Key, unsigned int key_len, char* Out, unsigned int out_len, const char cvecstr[8])  
{  
    int i,j,k;  
   ...
   //  return 0;
    ...
    {  
        ...
        {  
            /*密钥长度不对*/  
            printf("Run3Des failure(ret:%d) key:%d\n",6,nKey);
            return 0;  
        }  
    }  
    else  
...
/******************************************************************* 
  ...
  更 新 日 期：  2009.6.17 
*******************************************************************/  
int AES::RunPad(int nType,const char* In,unsigned in_len,char* Out,int* padlen)  
{  
...
int AES::CovertKey(char *iKey, char *oKey)  
{  
    char in[64],out[64];  
    int  inlen,i,j;  
...
void AES::MyDesInit(int* cryptmode,int* padmode,char* cvec)  
{  
    char buf[10];  
...
*******************************************************************/  
int AES::Crypt3Des(int type,char* in,int nsrclen,char* key,char* out,int & outLen)
{
   ...
   // if (inlen % 8)
    ...
    if (type==ENCRYPT)
    {
        ...
        printf("Run3Des inlen %d\n",inlen);
        if (!RunPad(padmode,in,inlen,instr,&inlen))
            return -3;

        printf("Run3Des inlen %d\n",inlen);
        if (!Run3Des(0,cryptmode,instr,inlen,key,(unsigned char)keylen,outstr,sizeof(outstr),cvec))
            return -4;
      ...
      //  strcpy(out,outstr);
        ...
        outlength = inlen;
        memcpy(out,outstr,inlen);
        outLen = inlen;
      //  Base64Encode(out,outstr,outlength);
        //printf("Run3Des %s\n",out);
    ...
    else if (type==DECRYPT)
    {
       ...
       //free(q);
        ...
        if (!Run3Des(1,cryptmode,in,inlen,key,(unsigned char)keylen,outstr,sizeof(outstr),cvec))
            return -4;
        RunRsm(outstr);
        strcpy(out,outstr);
...
unsigned char AES::GetByte(char *s)  
{  
    int                v1;  
    int                v2;  
