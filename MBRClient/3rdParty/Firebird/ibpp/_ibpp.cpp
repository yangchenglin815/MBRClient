...
GDS* GDS::Call()
{
	...
	if (! mReady)
	{
		...
		if (mHandle == 0)
		{
			...
			if (len != 0)
			{
				...
				*p = '\0';
				...
				{
					*p = '\0';
					lstrcat(fbdll, "\\fbembed.dll");	// Or possibly renamed fbclient.dll
					mHandle = LoadLibrary(fbdll);
				}
			}
		}
		...
		if (mHandle == 0)
		{
			...
			mHandle = LoadLibrary("fbembed.dll");
			if (mHandle == 0)
			{
				...
				if (mHandle == 0)
					throw LogicExceptionImpl("GDS::Call()",
						_("Can't find or load FBEMBED.DLL or GDS32.DLL"));
			}
		}
#endif
